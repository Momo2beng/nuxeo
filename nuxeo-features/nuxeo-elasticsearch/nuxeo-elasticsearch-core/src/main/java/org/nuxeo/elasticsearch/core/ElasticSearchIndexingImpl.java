            ...
            try {
                ...
                if (idxRequest != null) {
                    ...
                    if (secondaryIndex != null) {
                        IndexRequest idxRequestBis = new IndexRequest(secondaryIndex, DOC_TYPE,
    ...
    protected void processIndexRequest(String documentId, IndexRequest request) {
        if (log.isDebugEnabled()) {
            logDebugMessageTruncated(String.format("Index request: curl -XPUT 'http://localhost:9200/%s/%s/%s' -d '%s'",
                    request.index(), DOC_TYPE, documentId, request), MAX_CURL_LINE);
        }
        try {
            esa.getClient().index(request);
        } catch (ConcurrentUpdateException e) {
            SequenceTracer.addNote("Ignore indexing of doc " + documentId);
